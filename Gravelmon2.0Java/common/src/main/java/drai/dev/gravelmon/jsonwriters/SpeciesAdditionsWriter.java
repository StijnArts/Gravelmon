...
public class SpeciesAdditionsWriter {
    ...
    private synchronized static void writePokemon(Map.Entry<String, List<Pokemon>> set, String dir) throws IOException {
        ...
        if (Pokemon.ADDITIONAL_EVOLUTIONS.containsKey(set.getKey())) {
            fileContents = getEvolutions(Pokemon.getCleanName(set.getKey()), Pokemon.ADDITIONAL_EVOLUTIONS.get(set.getKey()), fileContents);
        }
        if (Pokemon.ADDITIONAL_DROPS.containsKey(set.getKey())) {
            fileContents = getDrops(1, Pokemon.ADDITIONAL_DROPS.get(set.getKey()), fileContents);
        }
        ...
        if (!set.getValue().isEmpty()) {
            ...
            for (Pokemon pokemon : set.getValue()) {
                ...
                var evolutions =
                fileContents = getEvolutions(Pokemon.getCleanName(set.getKey()), pokemon.getEvolutions(), fileContents);
