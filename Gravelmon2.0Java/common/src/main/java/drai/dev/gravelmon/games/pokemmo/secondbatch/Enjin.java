package drai.dev.gravelmon.games.pokemmo.secondbatch;


import drai.dev.gravelmon.pokemon.amavi.*;
import drai.dev.gravelmon.pokemon.dhiome.*;
import drai.dev.gravelmon.pokemon.enjin.Enchantowl;
import drai.dev.gravelmon.pokemon.enjin.Teleprog;
import drai.dev.gravelmon.pokemon.enjin.regional.*;
import drai.dev.gravelmon.pokemon.mushi.*;
import drai.dev.gravelmon.pokemon.theos.*;
import drai.dev.gravelmon.pokemon.kuria.*;
import drai.dev.gravelmon.pokemon.ottora.*;
import drai.dev.gravelmon.pokemon.spion.*;
import drai.dev.gravelmon.pokemon.mastenia.*;
import drai.dev.gravelmon.pokemon.mongratis.*;
import drai.dev.gravelmon.pokemon.ultraspace.*;
import drai.dev.gravelmon.pokemon.urzavos.*;
import drai.dev.gravelmon.pokemon.terranov.*;
import drai.dev.gravelmon.pokemon.enjin.*;
import drai.dev.gravelmon.pokemon.mystis.*;
import drai.dev.gravelmon.pokemon.pastelisland.*;
import drai.dev.gravelmon.pokemon.neonintendo.*;
import drai.dev.gravelmon.pokemon.tokas.*;
import drai.dev.gravelmon.pokemon.panjaea.*;
import drai.dev.gravelmon.pokemon.cyare.*;
import drai.dev.gravelmon.pokemon.olysos.*;
import drai.dev.gravelmon.pokemon.nodorro.*;
import drai.dev.gravelmon.pokemon.orbis.*;
import drai.dev.gravelmon.pokemon.fliga.*;
import drai.dev.gravelmon.pokemon.ruskow.*;
import drai.dev.gravelmon.pokemon.otopo.*;
import drai.dev.gravelmon.pokemon.goetia.*;
import drai.dev.gravelmon.pokemon.ionos.*;
import drai.dev.gravelmon.pokemon.lagoone.*;
import drai.dev.gravelmon.pokemon.atlas.*;
import drai.dev.gravelmon.pokemon.avoris.*;
import drai.dev.gravelmon.pokemon.enriko.*;
import drai.dev.gravelmon.pokemon.ferroa.*;
import drai.dev.gravelmon.pokemon.hiza.*;
import drai.dev.gravelmon.pokemon.hub.*;
import drai.dev.gravelmon.pokemon.interregional.*;
import drai.dev.gravelmon.pokemon.isiah.*;
import drai.dev.gravelmon.pokemon.junnin.*;
import drai.dev.gravelmon.pokemon.lonava.*;
import drai.dev.gravelmon.pokemon.mythire.*;
import drai.dev.gravelmon.pokemon.nosto.*;
import drai.dev.gravelmon.pokemon.novrai.*;
import drai.dev.gravelmon.pokemon.oceane.*;
import drai.dev.gravelmon.pokemon.okeno.*;
import drai.dev.gravelmon.pokemon.orohnhavai.*;
import drai.dev.gravelmon.pokemon.qamor.*;
import drai.dev.gravelmon.pokemon.rica.*;
import drai.dev.gravelmon.pokemon.varitas.*;
import drai.dev.gravelmon.pokemon.aristos.*;
import drai.dev.gravelmon.pokemon.midamis.*;
import drai.dev.gravelmon.pokemon.norheim.*;
import drai.dev.gravelmon.pokemon.attributes.*;
import drai.dev.gravelmon.pokemon.attributes.conditions.*;

import java.util.*;
public class Enjin extends drai.dev.gravelmon.games.registry.Game {
    public Enjin() {
        super("Enjin");
    }

    @Override
    public void registerPokemon() {
        pokemon.add(new Mouldon());
        pokemon.add(new Fungrove());
        pokemon.add(new Saprodon());
//        pokemon.add(new Larvuse());   Missing Art
//        pokemon.add(new Nymflash());   Missing Art
//        pokemon.add(new Quivarner());   Missing Art
//        pokemon.add(new Mizukey());   Missing Art
//        pokemon.add(new Seamian());   Missing Art
//        pokemon.add(new Suisage());   Missing Art
        pokemon.add(new Gastrohm());
        pokemon.add(new Mollectric());
        pokemon.add(new Nudibolt());
        pokemon.add(new Hokuro());
        pokemon.add(new Dustrix());
//        pokemon.add(new Plumine());   Missing Art
//        pokemon.add(new Arborsa());   Missing Stats
//        pokemon.add(new Fumostrello());   Missing Stats
//        pokemon.add(new Salamare());   Missing Stats
//        pokemon.add(new Seefowl());   Missing Stats
//        pokemon.add(new Hamstorch());   Missing Stats
//        pokemon.add(new Hippop());   Missing Stats
//        pokemon.add(new Faunifer());   Missing Stats
//        pokemon.add(new Sirain());   Missing Stats
        pokemon.add(new Budbun());
        pokemon.add(new Hoothoot(163));
        pokemon.add(new Noctowl(164));
        pokemon.add(new Enchantowl());
//        pokemon.add(new Chimini());   Missing Stats
//        pokemon.add(new Aquamel());   Missing Stats
//        pokemon.add(new Cattack());   Missing Stats
        pokemon.add(new Feilex());
        pokemon.add(new Paintta());
        pokemon.add(new Piscasso());
//        pokemon.add(new Bistodum());   Missing Stats
//        pokemon.add(new Coldfin());   Missing Stats
        pokemon.add(new Raveclem());
//        pokemon.add(new Galats());   Missing Stats
//        pokemon.add(new Coldier());   Missing Stats
        pokemon.add(new Sableye(302));
        pokemon.add(new Eyeziant());
        pokemon.add(new Ghostea());
        pokemon.add(new Sinistea(854));
        pokemon.add(new Polteageist(855));
//        pokemon.add(new Bihaund());   Missing Stats
        pokemon.add(new Serpendage());
        pokemon.add(new Jerbalm());
//        pokemon.add(new Kiwatt());   Missing Stats
        pokemon.add(new Kikiwi());
//        pokemon.add(new Magmaraca());   Missing Stats
//        pokemon.add(new Magmaligna());   Missing Stats
//        pokemon.add(new Punky());   Missing Stats
//        pokemon.add(new Queenival());   Missing Stats
//        pokemon.add(new Roselot());   Missing Stats
//        pokemon.add(new Rosival());   Missing Stats
//        pokemon.add(new Roswain());   Missing Stats
        pokemon.add(new Snooz());
        pokemon.add(new Drowzee(96));
        pokemon.add(new Hypno(97));
//        pokemon.add(new Slamb());   Missing Stats
        pokemon.add(new Tenwreck());
        pokemon.add(new Snouturf());
//        pokemon.add(new Volverdi());   Missing Stats
        pokemon.add(new Curstar());
//        pokemon.add(new Shihtzoof());   Missing Stats
//        pokemon.add(new Priminpup());   Missing Stats
//        pokemon.add(new Cucuppy());   Missing Stats
        pokemon.add(new Gremelon());
        pokemon.add(new Skoopy());
//        pokemon.add(new Thornip());   Missing Stats
        pokemon.add(new Nostroot());
        pokemon.add(new Khthonion());
        pokemon.add(new Cocoward());
        pokemon.add(new Cocombat());
        pokemon.add(new Electrike(309));
        pokemon.add(new Manectric(310));
//        pokemon.add(new Lunectric());   Missing Stats
        pokemon.add(new Spikid());
        pokemon.add(new Matchoppa());
//        pokemon.add(new Wotter());   Missing Stats
//        pokemon.add(new Cyclub());   Missing Stats
//        pokemon.add(new Angeroid());   Missing Stats
        pokemon.add(new Teleprog());
//        pokemon.add(new Clampere());   Missing Stats
//        pokemon.add(new Balbuni());   Missing Stats
        pokemon.add(new Criseti());
//        pokemon.add(new Anseti());   Missing Stats
//        pokemon.add(new Macrochip());   Missing Stats
        pokemon.add(new Sloggin());
        pokemon.add(new Lucidris());
        pokemon.add(new Chlorodra());
        pokemon.add(new Vipuny());
        pokemon.add(new Vipentide());
        pokemon.add(new Pterabonk());
        pokemon.add(new Smashzal());
        pokemon.add(new Curdwaul());
//        pokemon.add(new Tardidrake());   Missing Stats
        pokemon.add(new Cubreeze());
        pokemon.add(new Gossnow());
        pokemon.add(new Frosstitan());
        pokemon.add(new Waddlur());
        pokemon.add(new Cuddlur());
        pokemon.add(new Abysprit());
        pokemon.add(new Marvelisk());
        pokemon.add(new Nuptielle());
//        pokemon.add(new Burielle());   Missing Stats
//        pokemon.add(new Aristocalf());   Missing Stats
//        pokemon.add(new Majestapir());   Missing Stats
    }

}
