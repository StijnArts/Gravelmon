            ...
            if (!Pokemon.isAnAdditionalForm(pokemon)) {
                ...
                if (true) {
                    try {
                        ...
                        pokemon.getEvolutions().add(new EvolutionEntry(getKeysByValue(ADDITIONAL_FORMS, pokemon).stream().toList().get(0) + aspect, EvolutionType.LEVEL_UP, List.of(), List.of()));
    ...
    private static String getEvolutions(Pokemon pokemon, String fileContents) {
        if (!pokemon.getEvolutions().isEmpty()) {
            ...
            for (EvolutionEntry evolution : pokemon.getEvolutions()) {
                ...
                fileContents += "    {\n" +
                        "      \"id\": \"" + pokemon.getCleanName() + "_" + Pokemon.getCleanName(evolution.getResult()) + "\",\n" +
